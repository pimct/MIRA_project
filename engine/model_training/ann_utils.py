...
def plot_parity(y_true_dict, y_pred_dict, output_cols, skip_index, save_path):
    ...
    ncols, nrows = 2, math.ceil(len(plot_indices) / 2)
    ...
    for ax_i, i in enumerate(plot_indices):
        ...
        for key in ["train", "val", "test"]:
            y_true = y_true_dict[key][:, i]
            y_pred = np.clip(y_pred_dict[key][:, i], 0, None)
            r2 = r2_score(y_true, y_pred)
            ax.scatter(y_true, y_pred, alpha=0.7, label=f"{key.capitalize()} (R²={r2:.2f})", color=colors[key])

        min_val = max(0, min(y_true_dict[k][:, i].min() for k in y_true_dict))
        max_val = max(y_true_dict[k][:, i].max() for k in y_true_dict)
        ax.plot([min_val, max_val], [min_val, max_val], 'k--')
        ax.set(xlabel="Actual", ylabel="Predicted", title=output_cols[i])
        ax.legend()
